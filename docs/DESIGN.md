...
| 부하 중 RSS | 노드·hub 각 ≤ 96 MB | `LOAD=1000`: 방문자 1,000명이 동시에 https로 링크를 친 직후 |
...
**힙 상한 `-R:MaxHeapSize=64m`.** native-image의 기본 힙 상한은 물리 메모리의 80%다. 그래서
Serial GC가 힙을 마음껏 늘리고 되돌려주지 않는다. 방문자 1,000명이 **다 떠난 뒤에도** 노드
RSS가 94 MB에 머물렀다. 상한을 박으면 같은 부하가 66 MB로 정착하고 속도는 같다(1,000/1,000,
0.5초). 이 값은 측정해서 골랐다.

| 노드 힙 상한 | 동시 1,000명 | 소요 | 부하 후 RSS |
|---|---|---|---|
| 기본(≈물리 80%) | 1,000/1,000 | 0.5초 | 94–106 MB |
| 128m | 1,000/1,000 | 0.5초 | 87 MB |
| **64m (채택)** | 1,000/1,000 | 0.5초 | 65–70 MB |
| 32m | 1,000/1,000 | **1.1초** | 50 MB |
| 24m | **1/1,000** | — | OutOfMemoryError |

절벽은 24m에 있고 채택값은 그 2.7배다. 동시 2,000명(2,000/2,000, 63 MB)과 4,000명(3,952/4,000,
67 MB, OOM 없음)까지 견디는 것을 확인했다. 4,000명에서 일부 실패하는 것은 상한과 무관하다.
기본 힙으로도 3,743/4,000이며, 오히려 상한 쪽이 더 성공했다. 그보다 큰 동시성을 받는 배포는
실행 시 `-Xmx`로 덮어쓴다(`jailhub serve -Xmx256m`). 덮어쓰기가 동작하는 것도 확인했다.

**아이들 RSS는 이 상한으로 줄지 않는다.** 24.5 MB는 힙 여유가 아니라 살아 있는 데이터다.
단계별로 재보면 데몬만 띄웠을 때 16.7 MB이고, hub에 붙는 순간 24.3 MB가 된다. 링크를 여는 것은
+0.2 MB, 방문자 한 명은 +0.6 MB에 불과하다. 즉 7.6 MB는 TLS 클라이언트와 JSSE 초기화가
차지한다. 20 MB 목표는 힙 튜닝이 아니라 그쪽을 건드려야 하며 §15에 남긴다.
...
- **부하 중 RSS** — **해결됨**: `-R:MaxHeapSize=64m`으로 노드 94 → 66 MB, hub 76 → 60 MB (§4).
  추측이 맞았다. 부하가 빠진 뒤 RSS가 돌아오지 않은 것은 Serial GC가 힙을 반납하지 않아서였고,
  기본 상한이 물리 메모리의 80%라 사실상 상한이 없었다. 순차 방문자 600명으로 확인했다. RSS가
  72 MB에서 평평해지므로 누수가 아니라 힙 여유다. 남은 후보는 방문자당 라이브 메모리 자체를
  줄이는 쪽(버퍼 풀링, §8의 256 KB 윈도우 축소)이며, 상한 절벽이 24m인 것으로 보아 동시
  1,000명의 라이브 데이터는 24~32 MB다.
- **노드 아이들 RSS 20 MB 회복** — 현재 24.6 MB. **힙 상한은 답이 아니라는 것이 측정으로
