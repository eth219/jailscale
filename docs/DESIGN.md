...
- **이름 재배정 통지** — 자기 탐침(§12.5)은 구현됐으나 노드가 `jailscale verify`를 칠 때만
  돈다. 재배정 공격은 hub이 저장소에서 이름을 옮겨야 성립하는데(§9.3 서명 조건 2), 지금은 이름을
  뺏긴 노드에게 아무 통지도 가지 않는다(`Links.open`이 조용히 `byId.remove`). 정직한 hub이라면
  통지를 보내므로 침해된 hub을 잡지는 못하지만, 사고를 즉시 알 수 있게 한다. 탐침을 주기적으로
  돌리는 쪽과 함께 설계 필요.
