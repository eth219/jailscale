...
| 부하 중 RSS | 노드 ≤ 192 MB, hub ≤ 160 MB | `LOAD=1000`: 방문자 1,000명을 **동시에 열어 둔 채** 측정한 최대값 |
...
원래 목표였던 노드 20 MB는 §15에 남긴다(현재 24.6 MB). 게이트는 현재 실측에
여유를 둔 값이며, 넘으면 스크립트가 실패한다. 예산은 이유와 함께 PR로만 바꾼다.

**부하 측정은 연결을 열어 둔 채 한다.** 원래 게이트는 `curl --parallel`로 짧은 요청
1,000건을 쏘고 끝났다. 그러면 1,000개가 **동시에 살아 있던 적이 없다.** 실제 메모리의 3분의 1을
재고 있었다. 지금은 `tools/hold-visitors.py`가 1,000개를 모두 열어 둔 상태로 유지하고, 그동안의
최대 RSS를 표본으로 잡는다. 커널 accept 큐보다 큰 버스트는 jailscale에 닿기 전에 커널이
리셋하므로 100개씩 나눠 올린다.

**힙 상한은 두지 않는다.** 한때 `-R:MaxHeapSize=64m`을 박았다가 되돌렸다. 근거로 삼은 측정이
틀렸기 때문이다. 짧은 요청 부하에서는 상한이 공짜처럼 보였다(94 → 66 MB, 속도 동일). 그러나
연결을 실제로 열어 둔 채 재면 64m은 동시 1,000개 중 716개에서 `OutOfMemoryError`를 낸다.

| 노드 힙 상한 | 동시 유지 1,000개 | 노드 최대 RSS |
|---|---|---|
| 64m | **716/1,000, OOM** | 105 MB |
| 128m | 1,000/1,000 | 148 MB |
| 256m | 1,000/1,000 | 144 MB |
| 상한 없음 | 1,000/1,000 | 217 MB |

고정 상한이 애초에 틀린 이유는 따로 있다. **동시성이 이름 수에 비례**하기 때문이다. hub은
이름당 1,024개까지 받고(`SniRouter.MAX_PER_NAME`, §9.1), 노드는 이름을 20개까지 열 수 있다
(§9.2). 즉 한 노드의 상한은 20,480 동시 연결이고, hub은 이름 수만큼 더 곱해진다. 어떤 고정
바이트 수도 이 범위를 옳게 덮지 못한다. 메모리를 묶는 올바른 손잡이는 힙이 아니라 이 연결
한도다. 메모리가 빠듯한 호스트는 운영자가 `-Xmx`로 직접 정한다(`jailhub serve -Xmx256m`).

**대신 알게 된 것.** 부하가 빠진 뒤 RSS가 돌아오지 않는 것은 Serial GC가 힙을 반납하지 않기
때문이 맞다. 순차 방문자 600명 뒤 72 MB에서 평평해진다(누수가 아니라 힙 여유). native-image의
기본 힙 상한이 물리 메모리의 80%라 사실상 상한이 없는 것도 사실이다. 다만 그 대가로 용량을
깎는 것은 서버 프로세스에서 나쁜 거래다. 메모리를 더 쓰는 것과 죽는 것은 같은 급의 문제가 아니다.

**아이들 RSS는 힙과 무관하다.** 24.7 MB는 힙 여유가 아니라 살아 있는 데이터다. 단계별로 재면
데몬만 띄웠을 때 16.7 MB이고, hub에 붙는 순간 24.3 MB가 된다. 링크를 여는 것은 +0.2 MB, 방문자
한 명은 +0.6 MB다. 즉 7.6 MB는 TLS 클라이언트와 JSSE 초기화가 차지한다. 상한을 16m으로 낮춰도
이 값은 그대로다. 20 MB 목표는 힙이 아니라 그쪽을 건드려야 하며 §15에 남긴다.
...
- 명령: `up`, `down`, `status`, `open`, `close`, `ls`, `gate`, `invite`, `admin`, `netcheck`,
  `verify`(§12.5), `leave`, `service install|uninstall|status`.
...
|---|---|
| 정상 노드로 가는 방문자 트래픽 읽기 (노드가 종단한다) | **이름을 공격자 노드로 재배정**하고 와일드카드 키로 서명 → 그 이름의 방문자를 통째로 가로챈다 (막지는 못하나 §12.5로 탐지된다) |
...
**막는 것과 아는 것은 다르다.** 이 재배정을 hub 쪽에서 막을 방법은 없다. 이름의 소유를 정하는
것이 hub 자신이기 때문이다. 그래서 §12.5는 막는 대신 **노드가 스스로 알아채게** 한다.
...
### 12.5 자기 탐침 — 노드가 hub의 종단을 알아챈다

§12.3의 재배정은 hub 쪽에서 막을 수 없다. 대신 노드가 **자기 이름에 직접 접속해 그 TLS를 자신이
종단했는지 확인한다.** `gosuda/portal-tunnel`이 쓰는 방식이며, 여기서는 `jailscale verify`다.

근거는 RFC 5705 keying material이다. TLS 1.3 세션의 양 끝은 같은 레이블에서 같은 바이트를
유도하고, 그 세션을 종단하지 않은 제3자는 유도할 수 없다. 그래서:

1. 노드는 자신이 종단한 모든 방문자 세션의 export 값을 기록한다 (`SelfProbe.record`,
   `Visitors`의 핸드셰이크 직후). 최근 120초·최대 4,096개만 들고 있는다.
2. `jailscale verify`는 열려 있는 https 링크마다 **공개 이름으로** 방문자처럼 접속한다.
   주소는 `state.hubAddr`, SNI는 링크의 호스트다. 즉 hub를 실제로 통과한다.
3. 접속 후 `GET /`를 보내고 응답까지 받는다. 응답이 왔다는 것은 서버 쪽이 이미 핸드셰이크를
   마치고 기록했다는 뜻이다. 이 왕복이 1번과 4번 사이의 경합을 없앤다.
4. 클라이언트 쪽에서 export한 값이 기록에 있으면 `terminated by this node`,
   없으면 `TERMINATED ELSEWHERE`다. 비교는 `MessageDigest.isEqual`로 한다.

와일드카드 키를 쥔 hub가 그 이름을 스스로 종단하거나 다른 노드에 넘기면, 방문자가 보는 인증서는
여전히 유효하지만 세션이 달라 export 값이 기록에 없다. 읽고 다시 암호화하는 중계도 같은 이유로
잡힌다. 재암호화된 세션은 다른 세션이기 때문이다.

**한계.** 탐침은 노드 자신의 주소에서 나간다. 그 연결만 골라 진짜 노드로 보내는 hub는 잡히지
않는다. 다만 그러려면 hub가 방문자를 **차별**해야 하고, 그 자체가 탐지 가능한 행동이다.
`--tls-insecure`나 잘못된 `--ca-file`을 쓰면 탐침도 같이 눈이 먼다.

**자바에서 되는가.** 된다. `ExtendedSSLSession.exportKeyingMaterialData`가 JDK 25에 있다.
`SSLSession`만 보면 없는 것처럼 보인다. 설계 문서는 한동안 "자바에서는 불가능"이라고 적고
있었고, 그것은 `SSLSession`의 메서드만 열거한 데서 나온 잘못된 결론이었다. 리플렉션이 아니므로
§4의 native image 규율과 충돌하지 않는다. **네이티브 바이너리에서 실제로 확인했다**: 루프백
hub·노드를 native로 띄우고 `jailscale verify`를 쳐서 `terminated by this node`를 받았다. JVM에서
되는 것이 native image에서 된다는 보장이 없으므로 이 확인이 필요했다. TLS 1.2 이하는 이 API가
예외를 던지며, 그때 판정은 `keying material unavailable`이다.

**§12.1과의 관계.** 서명 4조건은 **hub가** 강제한다. 그래서 악성 *노드*를 막고 악성 *hub*에는
아무 말도 하지 않는다. 자기 탐침은 **노드가** 실행한다. 둘은 겹치지 않고 서로 반대쪽을 본다.

### 12.6 이름 재배정 통지

자기 탐침(§12.5)은 노드가 `jailscale verify`를 칠 때만 돈다. 아무도 치지 않으면 아무것도 모른다.
그래서 **정직한 hub은 이름이 옮겨갈 때 먼저 알린다.** `LinkRevoked{linkId, name, reason, at}`이다.
`reason`은 `reassigned`(다른 노드가 같은 이름을 열었다) 또는 `released`(운영자가 회수했다)다.

**무엇을 막지 못하는지 먼저.** 침해된 hub은 이 통지를 그냥 안 보내면 된다. 이것은 공격 탐지가
아니라 **사고 통지**다. 침해된 hub을 잡는 것은 §12.5이고, 둘은 목적이 다르다.

**트리거는 살아 있는 링크가 아니라 저장소의 소유권이다.** 이 구분이 핵심이다. 이름을 빼앗기는
노드는 대개 **오프라인**이고, 오프라인인 것이 바로 남이 그 이름을 가져가는 이유다. 그 순간
`byName`에는 아무 링크도 없다. 그래서 `Links.open`은 `NameRec.mkey`가 바뀌는지를 보고, 바뀌면
이전 mkey에게 알린다. 도메인은 `DomainRec`, raw 포트는 `PortRec`로 같은 판단을 한다.

**전달.** 그 mkey의 노드가 붙어 있으면 바로 보낸다. 없으면 hub 저장소에 쌓아 두고
(`notice-added`), 그 노드가 다음에 붙을 때 연결 0에서 넘긴 뒤 지운다(`notices-cleared`).
전부 보낸 다음에 지우므로, 전달 도중 죽은 노드는 다음에 다시 듣는다. 돌아오지 않는 노드가
상태를 무한히 키우지 않도록 노드당 20건까지만 들고 있고 넘으면 오래된 것부터 버린다.

**노드 쪽.** 받으면 그 이름을 자기 상태에서 지운다. 지우지 않으면 다음 재접속 때 조용히 다시
열려 통지가 무의미해진다. 그리고 `status`가 계속 말하도록 `revoked`에 남긴다. 로그 한 줄은
스크롤되어 사라지기 때문이다. 사용자가 그 이름을 **일부러 다시 열면** 경고를 지운다. 그것이
경고에 대한 대답이다. 이름으로 먼저 맞추고, raw 포트처럼 노드 쪽에 이름이 없는 경우에만
`linkId`로 맞춘다.

**운영자 회수.** `name release`·`domain release`는 그동안 저장소의 소유권만 지웠고 살아 있는
링크는 그대로 서비스했다. 이제 링크를 내리고 통지까지 보낸다. 다만 회수는 금지가 아니다.
이름이 비었으므로 같은 노드가 다시 열 수 있다. 노드를 막는 수단은 §12.4대로 노드 제거다.
...
`gosuda/portal-tunnel`의 자기 탐침을 따라 §12.5를 구현했다. 이 문서는 그전까지 "자바에는 RFC
5705 exporter가 없어 불가능"이라고 적고 있었고, 그것이 틀렸다는 것이 출발점이었다. 단위 테스트
4개(양 끝이 같은 값을 얻는가, 다른 세션이 다른 값을 얻는가, 남의 값을 거부하는가, 창이 오래된
것을 버리는가)와 e2e 1개(`LinkEndToEndTest`가 실제 링크를 열고 `verify`를 친다)가 붙었다.
**반증도 했다.** 기록값을 위조하도록 노드를 고치니 e2e가 두 링크 모두 `TERMINATED ELSEWHERE`로
판정하고 실패했다. 통과가 배선 때문이 아니라 실제 비교 때문임을 이렇게 확인했다. 네이티브
바이너리에서도 한 번 돌렸다. 예산 게이트는 그대로 통과한다(`LOAD=1000 ./measure.sh --check`:
바이너리 25.0/25.3 MiB, 아이들 24.9/24.6 MB, 부하 후 75.9/94.1 MB, CLI 5.6 ms).
...
- **탐침 자동 실행** — 자기 탐침(§12.5)과 재배정 통지(§12.6)가 모두 구현됐지만, 탐침은 여전히
  사람이 `jailscale verify`를 칠 때만 돈다. 주기 실행이 다음 단계다. 주기를 짧게 하면 hub에
  자기 트래픽이 늘고, 길게 하면 탐지가 늦다. 열어 둔 이름 수에 비례하므로 간격은 이름 수와
  함께 정해야 한다. 통지와 달리 탐침은 침해된 hub도 잡으므로, 자동화의 값어치는 이쪽이 크다.

  기록: 이 항목은 한동안 "RFC 5705 exporter가 자바에 없어 탐침이 불가능하다"고 적혀 있었다.
  `SSLSession`의 메서드만 열거한 잘못된 확인이었다. `ExtendedSSLSession.exportKeyingMaterialData`가
  있고, 탐침은 §12.5로 구현됐다.
...
- **방문자당 메모리** — 동시 1,000개를 열어 둔 채 노드 최대 RSS 87 MB, hub 77 MB(§4 게이트).
  아이들이 24.7 MB이므로 방문자 하나가 노드에서 약 60 KB를 차지한다. 대부분 `TlsEndpoint`가
  연결마다 잡는 `ByteBuffer` 3개(패킷 2개 + 애플리케이션 1개, 각 16 KB 남짓)와 mux 스트림
  버퍼다. 줄이는 후보는 버퍼 풀링과 §8의 256 KB 윈도우 축소. 이름당 1,024개(§9.1)가 상한이므로
  한 이름이 최대로 붐빌 때의 비용이 이 값에 1,024를 곱한 것이다. 힙 상한으로 누르려 했다가
  되돌린 경위는 §4에 적었다.
- **노드 아이들 RSS 20 MB 회복** — 현재 24.7 MB. **힙 상한은 답이 아니라는 것이 측정으로
  밝혀졌다**: 상한을 64m으로 박아도, 16m으로 낮춰도 아이들 RSS는 24.5 MB 그대로다. 단계별
  실측(§4)에 따르면 데몬만 띄운 상태가 16.7 MB이고 hub에 붙는 순간 24.3 MB가 된다. 남은 7.6 MB는
  TLS 클라이언트 한 개를 세우는 데 드는 JSSE 초기화다. 그래서 후보는 빌드 시 초기화 화이트리스트
  확대와 사용하지 않는 TLS 스위트·프로토콜 제거뿐이며, 둘 다 §4의 규율과 부딪히지 않는지 먼저
  봐야 한다. 20 MB는 데몬만 놓고 보면 이미 달성돼 있다는 점도 같이 적어 둔다.
