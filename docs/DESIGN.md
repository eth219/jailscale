...
- **릴레이의 TLS 종단 탐지** — §12.3이 받아들인 공격(침해된 hub이 이름을 재배정하고 와일드카드
  키로 서명)을 탐지하는 장치. `gosuda/portal-tunnel`은 실제 트래픽 시작 후 자기 연결을 탐침해
  양쪽에서 export한 TLS keying material을 대조하고 불일치를 릴레이 종단으로 본다. **그 방식은
  자바에서 그대로 쓸 수 없다**: JDK 25의 `SSLSession`·`SSLSocket`·`SSLEngine` 어디에도 RFC 5705
  exporter 공개 API가 없고(확인함), `SSLSession.getId()`는 와이어의 `legacy_session_id`가 아니라
  JSSE 내부 값이라 양쪽이 다르다(확인함). 내부 API는 §4의 리플렉션 금지와 충돌한다.
  응용 계층 논스 탐침은 구현 가능하지만, 탐침이 노드 자신의 IP에서 나오므로 침해된 hub이
  탐침만 진짜 노드로 보내 회피할 수 있어 보장이 약하다. **더 값어치 있어 보이는 방향은 이름
  재배정 통지다.** 이 공격은 hub이 저장소에서 이름을 옮겨야 성립하는데(§9.3 서명 조건 2),
  지금은 이름을 뺏긴 노드에게 아무 통지도 가지 않는다(`Links.open`이 조용히 `byId.remove`).
  노드가 "내 이름이 넘어갔다"를 알 수 있게 하는 편이 TLS를 탐침하는 것보다 직접적이다. 설계 필요.
