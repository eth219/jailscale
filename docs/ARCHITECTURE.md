...
instead -- which is the whole of what these settings buy. A hub somebody runs for themselves has
...
monitor that can read a fleet's versions and not its protocols cannot see a flag day coming and has
no release note mapping one to the other (§5.4).
