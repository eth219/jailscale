...
and `budget` run on the pull request head, on the runners and with the arguments `main` uses — the
job in `ci.yml` is one definition, so the two cannot drift. That is better evidence than the local
column below, which measures your laptop, and it is the one that costs you nothing to wait for.
Add it and the run starts; it does not need a push. The local command stays in the table because a
seven-minute round trip is a poor way to iterate, and because `measure.sh` prints figures the CI
gate only checks.
...
| the JDK or GraalVM pin | `./mvnw -pl node -am test -Dtest=TranscriptTest -Dsurefire.failIfNoSpecifiedTests=false`, on the new toolchain, pasting the `Tests run:` line — that flag means a mistyped class prints `BUILD SUCCESS` and runs nothing | `test` runs on Liberica, not on the pin, and the job that does (`budget`) is one of the two above — and `ci:full` would run it on GitHub's toolchain, not on the one you are moving to, so this row is the one the label cannot do for you. Delegated signing predicts the bytes JSSE writes (§9.2); a JDK that writes them otherwise takes every hub-signed handshake down. CONTRIBUTING.md has the long form |
...
4. Step 7, labels included, and `ci:full` as well when the change touches a row of that table. Then
   the merge rule below waits on `load` and `budget` like any other check, which is the whole
