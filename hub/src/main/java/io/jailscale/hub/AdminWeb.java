...
final class AdminWeb {
    ...
    private static String page(String title, String body) {
        return "<!doctype html><html lang=\"en\"><head><meta charset=\"utf-8\">"
            + HttpFront.NOINDEX
            // The same icon as the pages out front, and the same constant, so there is one place it
            // is written. Without the link a browser falls back to asking for /favicon.ico, which is
            // answered -- but with nosniff and an SVG body, which some browsers will not draw from
            // that name.
            + HttpFront.ICON
