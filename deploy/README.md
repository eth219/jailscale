...
| `nginx-stream.conf` | 이미 nginx가 443을 쥔 서버에 hub을 얹을 때. `ssl_preread` SNI 라우팅 + PROXY 헤더 |
...
프록시 뒤에 둘 때 hub은 반드시 `--proxy-protocol`과 함께 루프백에 리슨하거나 `--trusted-proxy <cidr>`를 줘야 한다. 그렇지 않으면 방문자 주소를 아무나 위조할 수 있으므로 hub이 기동을 거부한다.
