...
are plain builds on Liberica NIK (ARCHITECTURE.md §3.2). These are here so that anyone who wants a
...
That needs Oracle GraalVM for JDK 25 -- PGO is not in the Community Edition and not in Liberica --
...
| | plain (what ships) | with these profiles |
|---|---|---|
| Binary, linux-amd64 | 31.8, 32.0 MiB | **21.9, 21.9** |
| Idle RSS, linux-amd64 | 39.4, 38.6 MB | **32.4, 32.0** |
| Warm requests a second, linux-amd64 | 8,419 | **19,169** |
| Warm requests a second, arm64 macOS | **34,563** | 28,976 |
...
So: worth it if you run a hub on linux-amd64 and care about its footprint; not worth putting the
GraalVM Free Terms and Conditions on every released binary, nor worth a doubled release build, for
a gain that only lands on one of the five targets.
