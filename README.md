...
as it happens, and its page says whether it is in sync. With two hubs, let them
answer the DNS themselves: delegate the subdomain to both instead of the three
records above.

```
jailscale.example.com.      NS  ns1.jailscale.example.com.
jailscale.example.com.      NS  ns2.jailscale.example.com.
ns1.jailscale.example.com.  A   203.0.113.10
...
host is lost, every open name keeps working through the second, and the second
promotes itself once its nodes confirm the first is gone -- they carry a proof
the first has to answer and no node can forge -- which brings back joining,
opening new names and the admin page; nodes reconnect on their own, since the
hub key they pinned is the same. That self-promotion is off by default on a
hub with open registration, where `jailhub promote` is the way. The status
page of either shows how much of the last day, week and month it was up, by
its own record and as seen from the other
([ARCHITECTURE.md §13.1 to §13.5](docs/ARCHITECTURE.md)).
...
same gate held on each release commit. The budget builds with the toolchain and options the
