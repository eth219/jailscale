...
v0.1.3 carries four targets for both programs: `linux-amd64`, `linux-arm64`,
...
```sh
base=https://github.com/eth219/jailscale/releases/download/v0.1.3
...
them too. `:v0.1.3` pins that tag, `:latest` follows releases (it moves when
...
```
docker pull ghcr.io/eth219/jailhub:v0.1.3
docker pull ghcr.io/eth219/jailscale:v0.1.3
```
...
docker run -d --name jailscale --network demo \
    -v jailscale-state:/var/lib/jailscale ghcr.io/eth219/jailscale:v0.1.3
...
The figures are v0.1.2's; the release [Install](#a-binary) downloads is v0.1.3,
which changed the hub's control plane (a standby, the availability record) and
no path a visitor's bytes or an idle process take, and the same gate held on
its release commit. The budget builds with the toolchain and options the
...
- Upgrades have been exercised once each way they have been tried, on one hub
  and one node. v0.1.3 is the fourth tagged release. The two before it each
  ...
  `WireFormatTest` against v0.1.0's own `Hello` line; v0.1.3 adds message
  types only two hubs exchange, which a node never sees. Both
